...
\begin{itemize}
  \item[a)] implementato nello use case \textbf{\footnote{add here the
  number}Show Project Page}.
  \item[b)] implementato negli use case \textbf{1.2 Make NodeTaskbox} e in
  \textbf{2.2 Make GanttTaskbox}.
  \item[c)] implementato nello use case \textbf{1.3 Show UserOptions}
\end{itemize}

\section{Diagrammi specifici (2.3)}
\begin{itemize}
  \item[a)] implementato nello use case \textbf{1.3 Show UserOption}
  \item[b)] missing
  \item[c)] implementato nello use case \textbf{1.3 Show UserOptions}
  \item[d)] implementato nello use case \textbf{1.3 Show UserOptions} e
  descritto in modo dettagliato nella sezione \textbf{7.1 UserOption’s
  Instances} del documento \textbf{Domain Model}
  \item[e)] implementato nello use case \textbf{1.3 Show UserOptions}
\end{itemize}

\section{Generazione di immagini e doc (2.4)}
\begin{itemize}
  \item[a)] missing
  \item[b)] missing\footnote{collegare comunque lo use case in commons
  rifattorizzato}
  \item[c)] implementato nello use case \textbf{1.3 Show UserOptions} e
  descritto in modo dettagliato nella sezione \textbf{7.1 UserOption’s
  Instances} del documento \textbf{Domain Model}
  \item[d)] implementato negli use case relativi alla generazione dei
  diagrammi\footnote{insert here the ids of these use case}
  \item[e)] missing
\end{itemize}
