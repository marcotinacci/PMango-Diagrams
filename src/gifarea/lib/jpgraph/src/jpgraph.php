...
// Ver:		$Id: jpgraph.php 298 2005-11-30 22:16:04Z ljp $
//
...
// PUBLIC METHODS	
	...
	if( $aPlot == null )
	    JpGraphError::Raise("Graph::AddY() You tried to add a null plot to the graph.");	
	...
	if( !$this->xscale->IsSpecified() ) {
		...
		for($i=0; $i < $n; ++$i ) {
		    ...
		    if( class_exists('BarPlot') ) {
			$cl = strtolower(get_class($p));
			if( ($p instanceof BarPlot) || empty($p->barcenter)) 
