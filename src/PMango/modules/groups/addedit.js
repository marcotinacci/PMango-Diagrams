...
---------------------------------------------------------------------------
*/
function addSetcap(form) { 
	var fl = form.setcap.length -1;
	var au = form.assignedsc.length -1;

	var users = "x";

	//build array of assiged users
	for (au; au > -1; au--) {
		users = users + "," + form.assignedsc.options[au].value + ","
	}

	//Pull selected setcap and add them to list
	for (fl; fl > -1; fl--) {
		if (form.setcap.options[fl].selected && users.indexOf( "," + form.setcap.options[fl].value + "," ) == -1) {
			t = form.assignedsc.length
			opt = new Option( form.setcap.options[fl].text, form.setcap.options[fl].value);
			form.group_setcap.value += form.setcap.options[fl].value+";";
			form.assignedsc.options[t] = opt
		}
	}
...
function removeSetcap(form) {
	fl = form.assignedsc.length -1;
	for (fl; fl > -1; fl--) {
		if (form.assignedsc.options[fl].selected) {
			//remove from members
			var selValue = form.assignedsc.options[fl].value;			
			var re = ".*("+selValue+";).*";
			var hiddenValue = form.group_setcap.value;
			if (hiddenValue) {	
				var b = hiddenValue.match(re);
				if (b[1]) {
					hiddenValue = hiddenValue.replace(b[1], '');
				}
				form.group_setcap.value = hiddenValue;
				form.assignedsc.options[fl] = null;
			}
		}
	}
}
