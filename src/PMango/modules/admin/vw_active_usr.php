...
---------------------------------------------------------------------------
 ...
 Title:      view active user

 File:       vw_active_usr.php
...
$q  = new DBQuery;
$q->addTable('users', 'u');
$q->addQuery('DISTINCT(u.user_id), user_username, user_last_name, user_first_name, user_email');
$q->addJoin('user_setcap', 'uset', 'u.user_id = uset.user_id');
//$q->addJoin('groups', 'g', 'g.group_id = uset.group_id');
//$q->addJoin('permissions', 'per', 'user_id = permission_user');//da modificare lui poi prende nel select permission_user
